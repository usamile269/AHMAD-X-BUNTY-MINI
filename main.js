...
const { ahmadmd } = require('./lib/system');
const config = require('./config');
const events = require('./ahmad');
...
function ahmadLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [AHMAD-MD] ${new Date().toISOString()}: ${message}`);
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            ahmadLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');
            ...
            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                ahmadLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                ahmadLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function ahmadPair(number, res = null) {
    ...
    try {
        ...
        const ahmadStore = createahmadStore();
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'AHMAD-MD' };
            }
        });
        ...
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, ahmadStore);
        });
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© AHMAD-MD' } }
                    }},
        ...
        ahmadLog(`AHMAD-MD Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await ahmadPair(req.query.number, res); });
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Ahmad-md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 AHMAD-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
